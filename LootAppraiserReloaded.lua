...
-- Lua APIs
local select, tostring, time, unpack, tonumber, floor, pairs, tinsert, smatch,
      math, gsub = select, tostring, time, unpack, tonumber, floor, pairs,
                   table.insert, string.match, math, gsub

-- wow APIs
local GetContainerItemID, GetContainerItemInfo, GetUnitName, GetItemInfo,
      IsShiftKeyDown, GetBestMapForUnit, PlaySoundFile, GameFontNormal,
      RegisterAddonMessagePrefix, IsInGroup, UnitGUID, SecondsToTime,
      StaticPopupDialogs, StaticPopup_Show, SendAddonMessage =
    GetContainerItemID, GetContainerItemInfo, GetUnitName, GetItemInfo,
    IsShiftKeyDown, C_Map.GetBestMapForUnit, PlaySoundFile, GameFontNormal,
    C_ChatInfo.RegisterAddonMessagePrefix, IsInGroup, UnitGUID, SecondsToTime,
    StaticPopupDialogs, StaticPopup_Show, C_ChatInfo.SendAddonMessage
local INSTANCE_RESET_SUCCESS, OKAY, LOOT_ITEM_SELF, LOOT_ITEM_SELF_MULTIPLE =
    INSTANCE_RESET_SUCCESS, OKAY, LOOT_ITEM_SELF, LOOT_ITEM_SELF_MULTIPLE
        ...
        for slot = 1, C_Container.GetContainerNumSlots(bag) do
            ...
            if itemLink ~= nil then
                ...
                if itemID == nil then
                    ...
                    local name, _, rarity, _, _, _, _, _, _ =
                        GetItemInfo(itemID)
...
function private.PreparePricesources()
    ...
    if LA.Util.tablelength(priceSources) == 0 then
        ...
        if priceSource == "Custom" then
            ...
            local isValidCustomPriceSource =
                LA.TSM.ParseCustomPrice(LA.GetFromDb("pricesource",
                                                     "customPriceSource"))
...
function private.HandleItemLooted(itemLink, itemID, quantity, source)
    ...
    LA.Debug.Log(
        "  " .. tostring(itemID) .. ": " .. tostring(itemLink) .. " x" ..
            tostring(quantity)) -- gsub(itemLink, "\124", "\124\124");
    ...
    if LA.db.profile.general.useSubClasses == true then
        ...
        elseif class == "Consumable" and LA.db.profile.classTypeConsumable ==
            true then
        ...
        elseif class == "Tradeskill" and LA.db.profile.classTypeTradeskill ==
            true then
    ...
    if addItem2List == true then
        ...
        LA.UI.AddItem2LootCollectedList(itemID, itemLink, quantity, itemValue,
                                        false, source, disenchanted) -- add item
    ...
    if goldValue >= gatAValue and gatBValue == 0 and gatCValue == 0 then
        gatSoundToPlay = "A"
        LA.Debug.Log("  " .. tostring(itemID) .. ": gold value (" ..
                         tostring(goldValue) .. ") >= gold alert threshold (" ..
                         gatA .. ")")
        ...
        if not source or showGroupLoot then
            ...
            if tonumber(quantity) > 1 then
                ...
                LA:Pour(itemLink .. " x" .. quantity .. ": " .. formatValue ..
                            partyLootSuffix) -- this outputs to the channnel selected with qty value > 1
            ...
            if LA.GetFromDb("notification", "enableToasts") then
                local name, _, _, _, _, _, _, _, _, texturePath = GetItemInfo(
                                                                      itemID)
                -- LibToast:Spawn("LootAppraiserReloaded", name, texturePath, quality, quantity, formattedValue, source)
                LibToast:Spawn("LootAppraiserReloaded", name, texturePath,
                               quality, quantity, formatValue, source) -- updated to show qty value > 1
            end
        end
    ...
    if IsInGroup() == true and LA.GetFromDb("display", "showGroupLootAlerts") ==
        true then
        ...
        if LA.GetFromDb("notification", "playSoundEnabled") then
            ...
            if gatSoundToPlay == "A" then
                ...
                local soundNameA = LA.db.profile.notification.soundNameA or
                                       "None"
                PlaySoundFile(LSM:Fetch("sound", soundNameA), "master")
            elseif gatSoundToPlay == "B" then
                LA.Debug.Log("gatSound: " .. gatSoundToPlay)
                local soundNameB = LA.db.profile.notification.soundNameB or
                                       "None"
                PlaySoundFile(LSM:Fetch("sound", soundNameB), "master")
            elseif gatSoundToPlay == "C" then
                LA.Debug.Log("gatSound: " .. gatSoundToPlay)
                local soundNameC = LA.db.profile.notification.soundNameC or
                                       "None"
...
function private.sellGrayItemsToVendor()
    local sellGrayItemsValue = tostring((LA.Session.GetCurrentSession(
                                            "data_sellGrayItemsToVendor")) or 0)
    LA.Session.SetCurrentSession("data_sellGrayItemsToVendor",
                                 tostring(sellGrayItemsValue))
        ...
        if CanMerchantRepair() then -- Check to make sure merchant has repairing service available
            ...
            if RepairCost > 0 then -- merchant can repair gear
                if GetMoney() >= RepairCost then
                    RepairAllItems()
                    -- Show cost summary
                    LA.Debug.Log("Repair cost: " ..
                                     GetCoinTextureString(RepairCost))
...
function LA.IsItemBlacklisted(itemID)
    ...
    local result = LA.TSM.IsItemInGroup(itemID,
                                        LA.GetFromDb("blacklist", "tsmGroup"))
...
LA.METADATA = {VERSION = "2018." .. LA_API.GetVersion()}
