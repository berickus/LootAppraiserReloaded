--[[
    SessionHistoryUI.lua
    AceGUI-based UI for viewing and managing session history
]]
...
-- Lua APIs
local pairs, ipairs, tostring, date, floor, table, sort =
    pairs, ipairs, tostring, date, floor, table, sort
...
function private.CreateUI()
    ...
    btnExportAll:SetCallback("OnClick", function()
        private.ExportAllSessions()
    end)
    ...
    btnSaveCurrent:SetCallback("OnEnter", function(widget)
        ...
        GameTooltip:AddLine("|cffffffffSave the current active session to history|r")
        GameTooltip:Show()
    end)
    btnSaveCurrent:SetCallback("OnLeave", function()
        GameTooltip:Hide()
    end)
...
function private.RefreshSessionList()
    ...
    sort(sortedSessions, function(a, b)
    ...
    if #sortedSessions == 0 then
        ...
        emptyLabel:SetText("|cffaaaaaa" .. "No sessions recorded yet. Sessions will appear here after you farm!" .. "|r")
...
function private.ShowSessionTooltip(widget, session)
    ...
    GameTooltip:AddDoubleLine("Duration:", "|cffffffff" .. LA.SessionHistory.FormatDuration(session.duration) .. "|r")
    
    GameTooltip:AddLine(" ")
    
    -- Location info
    GameTooltip:AddDoubleLine("Zone:", "|cffffffff" .. (session.zoneName or "Unknown") .. "|r")
    GameTooltip:AddDoubleLine("Character:", "|cffffffff" .. (session.player or "Unknown") .. "|r")
    
    GameTooltip:AddLine(" ")
    
    -- Value info
    GameTooltip:AddDoubleLine("Total Value:", "|cffffd700" .. LA.SessionHistory.FormatValue(session.totalValue) .. "|r")
    if session.totalValueGroup and session.totalValueGroup > 0 then
        GameTooltip:AddDoubleLine("Group Value:", "|cffffd700" .. LA.SessionHistory.FormatValue(session.totalValueGroup) .. "|r")
    end
    if session.vendorSales and session.vendorSales > 0 then
        GameTooltip:AddDoubleLine("Vendor Sales:", "|cffffd700" .. LA.SessionHistory.FormatValue(session.vendorSales) .. "|r")
    end
    ...
    if session.settings then
        GameTooltip:AddLine(" ")
        GameTooltip:AddDoubleLine("Price Source:", "|cffaaaaaa" .. (session.settings.priceSource or "N/A") .. "|r")
    end
...
function private.ShowRenameDialog(session)
    LA.Debug.Log("SessionHistoryUI: Showing rename dialog for session " .. session.id)
    ...
    local currentName = session.name or LA.SessionHistory.GetSessionDisplayName(session)
