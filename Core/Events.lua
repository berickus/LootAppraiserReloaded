--[[
    Events.lua
    WoW event handlers: loot events, chat messages, addon messages, instance resets
]]
...
-- Wow APIs
local GetUnitName, UnitGUID, IsInGroup =
    GetUnitName, UnitGUID, IsInGroup

-- Lua APIs
local smatch, time, unpack, tostring =
    string.match, time, unpack, tostring
...
function Events.OnTradeSkillCrafted(event, data)
    ...
    if loottype then
        ...
        if not itemLink or not quantity then
            LA.Debug.Log("#### ignore event! data: " .. data .. ", type=" .. tostring(loottype))
            LA.Debug.Log("   itemLink=" .. tostring(itemLink) ..
                "; quantity=" .. tostring(quantity) ..
                "; source=" .. tostring(source) .. ")")
            return
...
function Events.OnChatMsgAddon(event, prefix, msg, type, sender)
    ...
    local v = {}
    for i = 1, #tokens do
        tinsert(v, LibParse:JSONDecode(tokens[i]))
    end
...
function Events.OnResetInfoEvent(event, msg)
    if event == "CHAT_MSG_SYSTEM" then
        if msg:match("^" .. resetmsg .. "$") then
            ...
            local instanceName = smatch(msg,
                INSTANCE_RESET_SUCCESS:gsub("%%s", "(.+)"))
