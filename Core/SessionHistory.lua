...
function SessionHistory.Initialize()
    ...
    if not LALoot.global.sessions then LALoot.global.sessions = {} end
    if not LALoot.global.nextSessionID then LALoot.global.nextSessionID = 1 end
...
function SessionHistory.StartTracking()
    ...
    if not LALoot or not LALoot.global then SessionHistory.Initialize() end
...
function SessionHistory.SaveSession()
    ...
    if totalValue == 0 and itemCount == 0 and totalKills == 0 then
        LA.Debug.Log(
            "Session has no data (no loot, no value, no kills), not saving")
        return
    end
    ...
    local sessionRecord = {
        id = sessionID,
        name = (existingRecord and existingRecord.name) or
            ("Session " .. sessionID),
        startTime = startTime,
        endTime = endTime,
        duration = duration,
        ...
        mapID = mapID,
        player = currentSession.player or
            (GetUnitName("player", true) .. "-" .. GetRealmName()),
        totalValue = totalValue,
        totalValueGroup = currentSession.livGroup or 0,
        itemCount = itemCount,
        noteworthyCount = LA.Util.tablelength(
            currentSession.noteworthyItems or {}),
        currencyLooted = currentSession.currencyLooted or 0,
        vendorSales = currentSession.vendorSoldCurrencyUI or 0,
        settings = {
            priceSource = currentSession.settings and
                currentSession.settings.priceSource or "Unknown",
            qualityFilter = currentSession.settings and
                currentSession.settings.qualityFilter or "1"
        },
        priceSource = currentSession.settings and
            currentSession.settings.priceSource or "Unknown",
        qualityFilter = currentSession.settings and
...
function SessionHistory.GetSession(sessionID)
    ...
    for _, session in ipairs(sessions) do
        if session.id == sessionID then return session end
    end
    return nil
end
...
function SessionHistory.DeleteSession(sessionID)
    ...
    for i, session in ipairs(LALoot.global.sessions) do
        if session.id == sessionID then
            ...
            LA:Print("Session deleted: " ..
                         (session.name or "Session " .. sessionID))
            return true
        end
    end
    return false
...
function SessionHistory.GenerateJSON(sessions)
    ...
    for _, session in ipairs(sessions) do
            ...
            for _, item in ipairs(session.loot) do
                ...
                if grouped[key] then
                    grouped[key].quantity =
                        grouped[key].quantity + (item.quantity or 1)
                    grouped[key].totalValue =
                        grouped[key].totalValue + (item.value or 0)
                else
                    grouped[key] = {
                        itemID = item.itemID,
        ...
        exportSessions[#exportSessions + 1] = {
            ...
            startTimeFormatted = date("%Y-%m-%d %H:%M:%S",
            ...
            priceSource = session.priceSource or
                (session.settings and session.settings.priceSource) or "Unknown",
            qualityFilter = session.qualityFilter or
                (session.settings and session.settings.qualityFilter) or "1",
            kills = killDetails,
            loot = lootItems
        }
    end
...
function SessionHistory.ShowHistoryWindow()
    ...
    private.historyUI:SetStatusText(
        "Left-click to rename | Right-click to export as JSON")
    ...
    private.historyUI:SetCallback("OnClose", function(widget) widget:Hide() end)
...
function SessionHistory.RefreshHistoryList()
    ...
    for _, session in ipairs(sortedSessions) do
        ...
        local text = string.format(
                         "|cFFFFD700%s|r - %s (%dm) - |cFFFFD700%dg|r - %d items",
                         session.name or ("Session " .. session.id), dateStr,
                         durationMin, totalGold, session.itemCount or 0)
        ...
        row:SetCallback("OnClick", function(widget, event, button)
            ...
            elseif button == "RightButton" then
                ...
                if json then SessionHistory.ShowExportPopup(json) end
            end
        end)
...
function SessionHistory.HandleCommand(input)
    ...
    if cmd == "reset" then
        ...
        StaticPopupDialogs["LA_HISTORY_RESET_CONFIRM"] = {
            ...
            button1 = "Yes",
            button2 = "No",
            OnAccept = function() SessionHistory.ResetAll() end,
            timeout = 0,
            whileDead = true,
            hideOnEscape = true
        }
...
function SessionHistory.SaveCurrentSession() return SessionHistory.SaveSession() end

-- Alias for ResetAll (returns count)
function SessionHistory.ResetAllSessions()
    if not LALoot or not LALoot.global then return 0 end
    local count = #(LALoot.global.sessions or {})
    LALoot.global.sessions = {}
    LALoot.global.nextSessionID = 1
    return count
...
function SessionHistory.GetSessionDisplayName(session)
    if session.name and session.name ~= "" then return session.name end
    return date("%Y-%m-%d %H:%M", session.startTime or 0) .. " - " ..
               (session.zone or session.zoneName or "Unknown")
end
...
function SessionHistory.ExportAllSessionsToJSON()
    local sessions = SessionHistory.GetAllSessions()
    if #sessions == 0 then return nil, "No sessions to export" end
