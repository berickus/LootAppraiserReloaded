...
LibToast:Register("LootAppraiserReloaded",
                  function(toast, text, iconTexture, qualityID, amountGained,
                           itemValue, source)
    local _, _, _, hex = GetItemQualityColor(qualityID)

    toast:SetFormattedTitle("|c%s%s|r %s", hex, text, amountGained and
                                _G.PARENS_TEMPLATE:format(amountGained) or "")

    if source then
        -- party loot
        -- LA:Print("party loot toast")
        toast:SetFormattedText("|cFF2DA6ED%s|r\n|cFFFFFFFF%s|r", source,
                               itemValue)
    else
...
function UI.ShowTimerWindow()
    ...
    private.TIMER_UI.frame:SetScript("OnUpdate", function(event, elapsed)
    ...
    private.TIMER_UI_TAB:SetPoint("BOTTOMLEFT", private.TIMER_UI.frame,
                                  "BOTTOMLEFT", 3, 3)
    private.TIMER_UI_TAB:SetScript("OnUpdate", function(self)
    ...
    if LA.Session.GetPauseStart() then
        LA.UI.b1:SetNormalTexture(
            [[Interface\Buttons\UI-SpellbookIcon-NextPage-Up]])
    else
        LA.UI.b1:SetNormalTexture([[Interface\TimeManager\PauseButton]])
    end
    LA.UI.b1:SetHighlightTexture([[Interface\Buttons\UI-Common-MouseHilight]],
                                 "ADD")
    LA.UI.b1:SetPoint("BOTTOMLEFT", l1, "BOTTOMLEFT", 4, 1)
    LA.UI.b1:SetScript("OnClick", function(self)
...
function UI.ShowLastNoteworthyItemWindow()
    ...
    private.LAST_NOTEWOTHYITEM_UI:SetStatusTable(LA.db.profile
                                                     .lastNotewothyItemUI)
...
function UI.ShowMainWindow(showMainUI)
    ...
    private.MAIN_UI.frame:SetScript("OnUpdate", function(event, elapsed)
        ...
        if mainUItotal >= 1 then
            ...
            -- set text
            local buttonresetInstance = private.MAIN_UI:GetUserData(
                                            "button_resetInstance")
    ...
    private.MAIN_UI:SetUserData("data_lootCollectedContainer",
                                lootCollectedContainer)
    ...
    private.MAIN_UI:SetHeight(baseHeight + listHeight + additionalButtonHeight +
                                  dataContainerHeight)
...
function UI.PrepareDataContainer(parent)
    ...
    if dataContainer then dataContainer:ReleaseChildren() end

    -- resize
    local lootCollectedContainerUI = private.MAIN_UI:GetUserData(
    ...
    ---------------------------------------------------------------------
    -- ...vendor sales value
    local formattedTotalLootedCurrency2 =
        LA.Util.MoneyToString(LA.Session.GetCurrentSession(
    ...
    local formattedTotalLootedCurrency =
        LA.Util.MoneyToString(LA.Session.GetCurrentSession("totalLootedCurrency") or
                                  0)
    ...
    -- ...looted item counter
    local lootedItemCounterUI = private.DefineRowForFrame(dataContainer,
                                                          "showItemsLooted",
                                                          "Items Looted:",
                                                          LA.Session
                                                              .GetCurrentSession(
    ...
    local noteworthyItemCounterUI = private.DefineRowForFrame(dataContainer,
                                                              "showNoteworthyItems",
                                                              "Noteworthy Items:",
                                                              LA.Session
                                                                  .GetCurrentSession(
...
function UI.RefreshStatusText()
    if private.MAIN_UI ~= nil then
        -- prepare status text
        local priceSourceAsText = LA.availablePriceSources[LA.db.profile
                                      .pricesource.source] or "undefined"
        local preparedText = LA.CONST.QUALITY_FILTER[tostring(LA.GetFromDb(
                                                                  "notification",
                                                                  "qualityFilter"))] -- filter
        preparedText = preparedText .. "|cffffffff | " .. priceSourceAsText or
                           tostring(LA.db.profile.pricesource.source) -- price source
...
function UI.RefreshUIs()
    ...
    if LA.Session.IsRunning() then
        ...
        if delta > 3600 then noSeconds = true end

        if LA.Session.GetPauseStart() then
            if time() % 2 == 0 then
                sessionDurationUI:SetText(" " ..
                                              SecondsToTime(delta, noSeconds,
                                                            false))
            else
                sessionDurationUI:SetText(" ")
            end
    ...
    -- looted item value
    local lootedItemValueUI =
        private.MAIN_UI:GetUserData("data_lootedItemValue")
    if LA.GetFromDb("display", "showLootedItemValue") and lootedItemValueUI then
        local totalItemValue = LA.Session.GetCurrentSession("liv") or 0
        local livValue = LA.Util.MoneyToString(totalItemValue)
        if LA.GetFromDb("display", "showXXXLootedItemValuePerHour") then
            livValue = livValue .. " (" ..
                           private.CalcLootedItemValuePerHour("liv") ..
                           "|cffffd100|r g/h)"
        end
    ...
    if LA.GetFromDb("display", "showCurrencyLooted") == true and
        currencyLootedUI then
        -- format the total looted currency and add to main ui
        local formattedValue = LA.Util.MoneyToString(LA.Session
                                                         .GetCurrentSession(
                                                         "totalLootedCurrency") or
                                                         0)
        currencyLootedUI:SetText(formattedValue)
    end

    -- vendor sold grays
    local currencyLootedUI2 = private.MAIN_UI:GetUserData(
                                  "data_vendorSoldCurrency")
    if LA.GetFromDb("display", "showValueSoldToVendor") == true and
        currencyLootedUI2 then
        -- format the total looted currency and add to main ui
        local formattedValue2 = LA.Util.MoneyToString(LA.Session
                                                          .GetCurrentSession(
                                                          "vendorSoldCurrencyUI") or
                                                          0)
    ...
    -- looted item counter
    local lootedItemCounterUI = private.MAIN_UI:GetUserData(
                                    "data_lootedItemCounter")
    if LA.GetFromDb("display", "showItemsLooted") and lootedItemCounterUI then
        -- add to main ui
        lootedItemCounterUI:SetText(LA.Session.GetCurrentSession(
                                        "lootedItemCounter") or 0)
    end

    -- noteworthy item counter
    local noteworthyItemCounterUI = private.MAIN_UI:GetUserData(
                                        "data_noteworthyItemCounter")
    if LA.GetFromDb("display", "showNoteworthyItems") and
        noteworthyItemCounterUI then
        -- add to main ui
        noteworthyItemCounterUI:SetText(LA.Session.GetCurrentSession(
    ...
    local lootedItemValueGroupUI = private.MAIN_UI:GetUserData(
                                       "data_lootedItemValueGroup")
    if LA.GetFromDb("display", "showLootedItemValueGroup") and
        lootedItemValueGroupUI then
        local totalItemValueGroup = LA.Session.GetCurrentSession("livGroup") or
                                        0
        local livValueGroup = LA.Util.MoneyToString(totalItemValueGroup)
        if LA.GetFromDb("display", "showLootedItemValueGroupPerHour") then
            livValueGroup = livValueGroup .. " (" ..
                                private.CalcLootedItemValuePerHour("livGroup") ..
                                "|cffffd100|r g/h)"
        end
    ...
    if UI.LITE_UI then
        ...
        private.LITE_UI:SetTitle("|cffffffff" ..
                                     LA.Util.MoneyToString(totalItemValue or 0) ..
                                     "|r")
    ...
    local buttonStopSessionUI =
        private.MAIN_UI:GetUserData("button_stopSession")
    if buttonStopSessionUI then
        if LA.Session.IsPaused() then
            buttonStopSessionUI:SetText("Restart")
            buttonStopSessionUI:SetCallback("OnClick",
                                            function()
                LA.Session.Restart()
            end)
        else
...
function UI.UpdateLastNoteworthyItemUI(itemLink, quantity, singleItemValue,
                                       formattedValue)
        ...
        if tonumber(quantity) > 1 then
            ...
            private.LAST_NOTEWOTHYITEM_UI:SetTitle(
                itemLink .. "|cffffffff x" .. quantity .. ": " ..
                    formattedItemValue .. "|r")
        else
            private.LAST_NOTEWOTHYITEM_UI:SetTitle(
                itemLink .. "|cffffffff x" .. quantity .. ": " .. formattedValue ..
                    "|r")
        end
...
function UI.ResetFrames()
    ...
    LA.db.profile.lastNotewothyItemUI = {
        ["height"] = 30,
        ["top"] = (parentHeight - 100),
        ["left"] = 50,
        ["width"] = 350
    }
    ...
    LA.db.profile.liteUI = {
        ["height"] = 30,
        ["top"] = (parentHeight - 135),
        ["left"] = 50,
        ["width"] = 150
    }
    if private.LITE_UI then private.LITE_UI:ApplyStatus() end

    LA.db.profile.timerUI = {
        ["height"] = 30,
        ["top"] = (parentHeight - 135),
        ["left"] = 290,
        ["width"] = 110
    }
    if private.TIMER_UI then private.TIMER_UI:ApplyStatus() end

    LA.db.profile.mainUI.top = parentHeight - 170
    LA.db.profile.mainUI.left = 50

    if private.MAIN_UI then private.MAIN_UI:ApplyStatus() end
end

function UI.GetTokenPrice() return LA.UI.WOW_TOKEN end
...
function private.OnBtnSellTrashClick()
    ...
    if merchantitems > 0 then
                ...
                if itemLink ~= nil then
                    ...
                    if itemID == nil then
                        ...
                        if rarity == 0 and currentItemValue ~= 0 then -- added currentItemValue
                            ...
                            if id and
                                LA.TSM
                                    .IsItemInGroup(id, LA.GetFromDb("sellTrash",
                                                                    "tsmGroup")) then
                                -- LA.Debug.Log("  id=" .. id .. ", found=" .. tostring(trashItems["i:" .. id]) .. ", link=" .. link)
                                C_Container.UseContainerItem(bag, slot)
                            else
        ...
        if LA.GetFromDb("display", "showValueSoldToVendor") == true then
            local formattedTotalVendorSoldCurrency = LA.Session
                                                         .GetCurrentSession(
                                                         "vendorSoldCurrencyUI") or
                                                         0
            ...
            LA.Debug.Log("Set Session: " ..
                             LA.Session
                                 .GetCurrentSession("vendorSoldCurrencyUI"))
...
function private.GetItemValue(itemID, priceSource)
    if (LA.Util.startsWith(LA.CONST.PRICE_SOURCE[LA.GetFromDb("pricesource",
                                                              "source")], "OE:") or
        (LA.Util.startsWith(
            LA.CONST.PRICE_SOURCE[LA.GetFromDb("pricesource", "source")], "AN:")) or
        (LA.Util.startsWith(
            LA.CONST.PRICE_SOURCE[LA.GetFromDb("pricesource", "source")],
            "BLIZ:"))) then
...
function private.IsDestroyBlacklistedItems()
    if not LA.TSM.IsTSMLoaded() then return false end
