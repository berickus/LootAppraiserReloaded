...
-- Lua APIs
local tostring, tonumber, select, time, string = tostring, tonumber, select,
                                                 time, string

--[[-------------------------------------------------------------------------------------
-- AceAddon-3.0 - module standard methods
---------------------------------------------------------------------------------------]]
function SalvageCrate:OnInitialize() LA.Debug.Log("SalvageCrate:OnInitialize()") end

function SalvageCrate:OnEnable()
    LA.Debug.Log("SalvageCrate:OnEnable()")

    -- events for opening of salvage crates
    SalvageCrate:RegisterEvent("UNIT_SPELLCAST_START",
                               SalvageCrate.onUnitSpellcast)
    -- SalvageCrate:RegisterEvent("UNIT_SPELLCAST_STOP", SalvageCrate.onUnitSpellcast)
    SalvageCrate:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED",
                               SalvageCrate.onUnitSpellcast)
    SalvageCrate:RegisterEvent("UNIT_SPELLCAST_INTERRUPTED",
                               SalvageCrate.onUnitSpellcast)
...
function SalvageCrate.onUnitSpellcast(event, unitID, spell, rank, lineID,
                                      spellID)

    if not LA.Session.IsRunning() then return end

    -- only salvage spells
    if not spellID or not salvageSpellIDs[tostring(spellID)] then return end
    ...
    if event == "UNIT_SPELLCAST_START" then
        ...
        LA.Debug.Log("    started: unitID=" .. unitID .. ", spell=" .. spell ..
                         ", rank=" .. rank .. ", lineID=" .. tostring(lineID) ..
    ...
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
        ...
        if currentLineID ~= lineID then return end

        LA.Debug.Log(
            "    succeeded: unitID=" .. unitID .. ", spell=" .. spell ..
                ", rank=" .. rank .. ", lineID=" .. tostring(lineID) ..
                ", spellID=" .. tostring(spellID))
    ...
    elseif event == "UNIT_SPELLCAST_INTERRUPTED" then
        ...
        LA.Debug.Log(
            "    interrupted: unitID=" .. unitID .. ", spell=" .. spell ..
                ", rank=" .. rank .. ", lineID=" .. tostring(lineID) ..
                ", spellID=" .. tostring(spellID))
...
function SalvageCrate.onBagUpdate(event, bagID)
    ...
    for slot = 1, GetContainerNumSlots(bagID), 1 do
        ...
        if oldValue ~= newValue then
            LA.Debug.Log("    different value for key=" .. key .. ", " ..
                             tostring(oldValue) .. " vs. " .. tostring(newValue))
